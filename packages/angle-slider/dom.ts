...
export const getRootId = (ctx: Scope) =>
  ctx.ids?.root ?? `angle-slider:${ctx.id}`;
export const getThumbId = (ctx: Scope) =>
  ctx.ids?.thumb ?? `angle-slider:${ctx.id}:thumb`;
export const getHiddenInputId = (ctx: Scope) =>
  ctx.ids?.hiddenInput ?? `angle-slider:${ctx.id}:input`;
